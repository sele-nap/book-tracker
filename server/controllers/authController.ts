...
export const postPublicRegister = asyncHandler(
...
export const postPublicLogin = asyncHandler(
  async (req: Request, res: Response) => {
    const { email, password } = req.body as { email: string; password: string };
...
export const postUserLogout = (_req: Request, res: Response) => {
  res.clearCookie('token', { ...COOKIE_OPTIONS, maxAge: 0 });
  res.status(204).send();
};

export const patchUserMe = asyncHandler(async (req: Request, res: Response) => {
...
export const deleteUserMe = asyncHandler(
  async (req: Request, res: Response) => {
