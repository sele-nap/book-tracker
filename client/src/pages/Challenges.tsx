...
function ChallengeCard({ challenge, onAddBook }: { challenge: Challenge; onAddBook: () => void }) {
  const { t } = useLanguage();
  const pct = Math.min(100, Math.round((challenge.books.length / challenge.goalBooks) * 100));
  const done = challenge.books.length >= challenge.goalBooks;

  return (
    <div className={`bg-dusk border rounded-2xl p-6 space-y-5 ${done ? 'border-amber/50' : 'border-mist/30'}`}>
      <div className="flex items-start justify-between">
        <div>
          <h3 className="font-display text-cream text-xl">{challenge.year}</h3>
          <p className="text-stone text-sm mt-0.5">
            {challenge.books.length} / {challenge.goalBooks} {t.challenges.books}
          </p>
        </div>
      ...
      <div>
        <div className="flex justify-between text-xs text-stone mb-1.5">
          <span>{pct}%</span>
          <span>{challenge.goalBooks - challenge.books.length > 0 ? `${challenge.goalBooks - challenge.books.length} ${t.challenges.remaining}` : t.challenges.completed}</span>
        </div>
      ...
      {challenge.targetGenres.length > 0 && (
        <div>
          <p className="text-xs text-parchment mb-2">{t.challenges.targetGenres}</p>
          <div className="flex flex-wrap gap-1.5">
            {challenge.targetGenres.map((g) => {
              const count = challenge.books.filter((b) => b.genre.includes(g)).length;
              return (
                <span key={g} className="flex items-center gap-1 text-xs bg-bark px-2.5 py-1 rounded-full text-parchment">
                  {g}
      ...
      <div>
        <p className="text-xs text-parchment mb-2">{t.challenges.recentBooks}</p>
        <div className="flex gap-1.5">
          {challenge.books.slice(-6).reverse().map((b) => (
            <div key={b._id} className="w-9 h-12 bg-bark rounded overflow-hidden shrink-0" title={b.title}>
              {b.coverUrl
                ? <img src={b.coverUrl} alt={b.title} className="w-full h-full object-cover" />
                : <span className="w-full h-full flex items-center justify-center text-sm opacity-20">📖</span>
              }
            </div>
          ))}
...
function AddBookModal({ challenge, onClose, onUpdate }: { challenge: Challenge; onClose: () => void; onUpdate: () => void }) {
  const { t } = useLanguage();
  ...
  return (
    <Modal title={`${t.challenges.addBook} — ${challenge.year}`} onClose={onClose}>
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {results.map((b) => (
              <div key={b._id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-bark/50">
                ...
                {alreadyIn.has(b._id) ? (
                  <span className="text-xs text-sage">✦ {t.challenges.alreadyIn}</span>
                ) : (
                  <button onClick={() => addBook(b._id)} className="text-xs text-parchment hover:text-cream transition-colors">
                    + {t.challenges.add}
                  </button>
                )}
              </div>
...
function CreateChallengeForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!goal) return;
    setLoading(true);
    await challengesApi.create({ year: parseInt(year), goalBooks: parseInt(goal), targetGenres: genres });
    setLoading(false);
    onSuccess();
  };

  const inputClass = 'w-full bg-bark border border-mist/20 rounded-lg px-3 py-2 text-cream placeholder:text-stone text-sm outline-none focus:border-mist/50';

  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-parchment mb-1">{t.challenges.year}</label>
          <input className={inputClass} type="number" value={year} onChange={(e) => setYear(e.target.value)} required />
        </div>
        <div>
          <label className="block text-xs text-parchment mb-1">{t.challenges.goal} *</label>
          <input className={inputClass} type="number" min={1} value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="52" required />
        </div>
      </div>

      <div>
        <label className="block text-xs text-parchment mb-1">{t.challenges.targetGenres}</label>
        <div className="flex gap-2">
          <input
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {genres.map((g) => (
              <span key={g} className="flex items-center gap-1 text-xs bg-mist/20 text-parchment px-2 py-0.5 rounded-full">
                {g}
                <button type="button" onClick={() => setGenres((gs) => gs.filter((x) => x !== g))} className="text-stone hover:text-cream">✕</button>
              </span>
            ))}
          </div>
        )}
      ...
      <button type="submit" disabled={loading} className="w-full bg-wine hover:bg-rose disabled:opacity-50 text-cream text-sm py-2.5 rounded-lg transition-colors">
...
export default function Challenges() {
  ...
  const { data: challenges, loading, refetch } = useApi<Challenge[]>(fetchChallenges);
    ...
    <div>
      {showCreate && (
        <Modal title={t.challenges.create} onClose={() => setShowCreate(false)}>
          <CreateChallengeForm onSuccess={() => { setShowCreate(false); refetch(); }} />
        </Modal>
      )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {challenges.map((c) => (
            <ChallengeCard key={c._id} challenge={c} onAddBook={() => setActiveChallenge(c)} />
          ))}
        </div>
      )}
    </div>
